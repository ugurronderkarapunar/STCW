"""
Main Dashboard Page
Displays KPIs, charts, and risk tables.
"""
...
def generate_sample_data() -> pd.DataFrame:
    ...
    ranks = [
        "Kaptan", "Başmühendis", "Vardiya Zabiti", "Lostromo",
        "Gemici", "Yağcı", "Aşçı", "Kamarot", "Makine Zabiti",
        "Elektrik Zabiti", "2. Kaptan", "3. Mühendis",
    ]

    documents = [
        "STCW", "SRC", "GOC", "Sağlık Raporu", "Gemiadamı Cüzdanı",
        "Pasaport", "Vize", "Güvenlik Eğitimi", "Yangın Eğitimi",
        "İlk Yardım", "Tanker Eğitimi", "ARPA", "ECDIS",
        "Liman Başkanlığı İzni", "İş Sağlığı ve Güvenliği",
    ]
    ...
    for i in range(n_records):
        ...
        # Generate expiry date with some randomness
        days_offset = np.random.choice([
            np.random.randint(-365, 0),    # Expired (30%)
            np.random.randint(0, 30),       # Critical (20%)
            np.random.randint(31, 90),      # Approaching (20%)
            np.random.randint(91, 730),     # Valid (25%)
            99999,                          # No date (5%) - will become None
        ])
...
def main():
    ...
    st.markdown(
        ...
        <div style='text-align: center; padding: 20px 0;'>
            <h1 style='color: #1B3A5C; margin-bottom: 0;'>📊 Personel Belge Takip Sistemi</h1>
            <p style='color: #7F8C8D; font-size: 1.1rem;'>
                Kurumsal Belge Yönetimi ve Risk Analizi Platformu
            </p>
        </div>
        <hr>
        """,
    ...
    if not st.session_state.data_loaded:
        ...
        with col2:
            st.markdown(
                """
                <div style='text-align: center; padding: 40px 20px; background: #FFFFFF; border-radius: 16px; box-shadow: 0 2px 12px rgba(0,0,0,0.08);'>
                    <h2 style='color: #1B3A5C;'>👋 Hoş Geldiniz</h2>
                    ...
                    <h4 style='color: #1B3A5C;'>🔍 Sistemin Sundukları</h4>
    ...
    if data_to_use is None or data_to_use.empty:
        st.warning("⚠️ Filtrelere uygun veri bulunamadı. Filtreleri genişletin.")
        return
    ...
    temp_processor.processed_data = data_to_use

    filtered_metrics = {
        "total_personnel": data_to_use["personnel_name"].nunique() if not data_to_use.empty else 0,
        "total_ranks": data_to_use["rank_normalized"].nunique() if not data_to_use.empty else 0,
    ...
    with tab1:
        ...
        with col1:
            ...
            render_document_type_chart(
                temp_processor.get_document_type_summary()
            )

        with col2:
            # Use full rank summary for charts (not filtered)
            ...
            if full_rank_summary is not None and not full_rank_summary.empty:
                ...
                st.info("Ünvan özeti bulunamadı.")
            ...
            if full_monthly is not None and not full_monthly.empty:
                ...
                st.info("Aylık öngörü bulunamadı.")
    ...
    with tab2:
        ...
        st.markdown("<div class='section-header'>⚪ Eksik Tarih Bilgisi</div>", unsafe_allow_html=True)
    ...
    with tab4:
        ...
        display_df.columns = [
            "Ad Soyad", "Ünvan", "Belge", "Bitiş Tarihi",
            "Kalan Gün", "Durum Emoji", "Durum"
        ][:len(available_cols)]
        ...
        with col1:
            ...
            st.download_button(
                ...
                data=csv_data,
                file_name=f"belge_raporu_{date.today().strftime('%Y%m%d')}.csv",
                mime="text/csv",
                use_container_width=True,
            )
        ...
        with col2:
            ...
            st.download_button(
                ...
                data=excel_data,
                file_name=f"belge_raporu_{date.today().strftime('%Y%m%d')}.xlsx",
        ...
        with col3:
            ...
            st.download_button(
                label="📥 Özet Metin İndir",
                data=summary_text,
                file_name=f"ozet_rapor_{date.today().strftime('%Y%m%d')}.txt",
                mime="text/plain",
                use_container_width=True,
            )
...
def render_personnel_cards(data: pd.DataFrame) -> None:
    ...
    for (name, rank), group in personnel_groups:
        ...
        st.markdown(
            ...
            <div class="personnel-card {risk_class}">
                ...
                <div class="document-tags">
                    {tags_html if tags_html else '<span style="color: #7F8C8D; font-size: 0.85rem;">✅ Tüm belgeler geçerli</span>'}
                </div>
                <div class="metric-row" style="margin-top: 10px;">
                    <div class="metric-item">
                        <div class="metric-value">{len(expired)}</div>
                        <div class="metric-label">🔴 Geçmiş</div>
                    </div>
                    <div class="metric-item">
                        <div class="metric-value">{len(critical)}</div>
                        <div class="metric-label">🟠 Kritik</div>
                    </div>
                    <div class="metric-item">
                        <div class="metric-value">{len(approaching)}</div>
                        <div class="metric-label">🟡 Yaklaşıyor</div>
                    </div>
                    <div class="metric-item">
                        <div class="metric-value">{len(group) - len(expired) - len(critical) - len(approaching)}</div>
                        <div class="metric-label">🟢 Geçerli</div>
                    </div>
                </div>
            </div>
            """,
        ...
        if count >= 50 and not search:
            st.info(f"İlk 50 personel gösteriliyor. Arama yaparak belirli bir personele ulaşabilirsiniz.")
            break
